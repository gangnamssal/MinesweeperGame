...
export default function useCheckMine() {
  ...
  const dr = useRef([-1, -1, 0, 1, 1, 1, 0, -1]); // 클릭한 부분 8방으로 탐색
  const dc = useRef([0, 1, 1, 1, 0, -1, -1, -1]); // 클릭한 부분 8방으로 탐색
  ...
  const checkMine = (mine: number[][], currentRow: number, currentCol: number) => {
    ...
    const deepCopyMine = _.cloneDeep(mine); // redux에 저장된 지뢰맵 클론
    const deepCopyVisited = _.cloneDeep(visited); // redux에 저장된 visited 클론
    let deepCopyStack = _.cloneDeep(stack); // redux에 저장된 stack 클론
    ...
    function helper() {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        if (nr >= 0 && nc >= 0 && nr < row && nc < col && !deepCopyVisited[nr][nc]) {
          if (deepCopyMine[nr][nc] === 10) cnt++; // 방문한 곳이 지뢰면 지뢰수를 체크
          arr.push([nr, nc]); // 방문했던 곳을 임시 저장
        }
      }
      ...
      } else deepCopyMine[r][c] = cnt; // 만약 지뢰가 하나라도 존재했다면 현재 위치에 지뢰 수를 표시
      helper();
    }
    helper();
